...
contract PairTest is BaseTest {
    ...
    function deployPairCoins() public {
        ...
        amounts[1] = 1e25;
        amounts[2] = 1e25;
        mintBpln(owners, amounts);
        mintLR(owners, amounts);

        VeArtProxy artProxy = new VeArtProxy();
        escrow = new VotingEscrow(address(BPLN), address(artProxy));
    }

    function createLock() public {
        deployPairCoins();

        BPLN.approve(address(escrow), 5e17);
        escrow.create_lock(5e17, 4 * 365 * 86400);
        vm.roll(block.number + 1); // fwd 1 block because escrow.balanceOfNFT() returns 0 in same block
        assertGt(escrow.balanceOfNFT(1), 495063075414519385);
        assertEq(BPLN.balanceOf(address(escrow)), 5e17);
    }
    ...
    function increaseLock() public {
        ...
        assertEq(BPLN.balanceOf(address(escrow)), TOKEN_1);
    }
    ...
    function votingEscrowMerge() public {
        ...
        BPLN.approve(address(escrow), TOKEN_1);
        escrow.create_lock(TOKEN_1, 4 * 365 * 86400);
        assertGt(escrow.balanceOfNFT(2), 995063075414519385);
        assertEq(BPLN.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(BPLN.balanceOf(address(escrow)), 3 * TOKEN_1);
        console2.log(escrow.totalSupply());
        escrow.merge(3, 1);
        console2.log(escrow.totalSupply());
        assertGt(escrow.balanceOfNFT(1), 1990063075414519385);
    ...
    function deployMinter() public {
        ...
        BPLN.setMinter(address(minter));
        address[] memory tokens = new address[](5);
        tokens[0] = address(USDC);
        tokens[1] = address(FRAX);
        tokens[2] = address(DAI);
        tokens[3] = address(BPLN);
        tokens[4] = address(LR);
    ...
    function deployPairFactoryGauge() public {
        ...
        BPLN.approve(address(gaugeFactory), 15 * TOKEN_100K);
        ...
        staking = new TestStakingRewards(address(pair), address(BPLN));
    ...
    function addGaugeAndBribeRewards() public {
        ...
        BPLN.approve(address(gauge), PAIR_1);
        BPLN.approve(address(xbribe), PAIR_1);
        BPLN.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(BPLN), PAIR_1);
        xbribe.notifyRewardAmount(address(BPLN), PAIR_1);
        staking.notifyRewardAmount(PAIR_1);

        assertEq(gauge.rewardRate(address(BPLN)), 1653);
        // no reward rate, all or nothing
        // assertEq(xbribe.rewardRate(address(BPLN)), 1653);
    ...
    function createLock2() public {
        ...
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(BPLN.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        BPLN.approve(address(voter), PAIR_1);
    ...
    function minterMint() public {
        ...
        console2.log(BPLN.balanceOf(address(distributor)));
        console2.log(distributor.claimable(1));
        uint256 claimable = voter.claimable(address(gauge));
        BPLN.approve(address(staking), claimable);
    ...
    function gaugeClaimRewards() public {
        ...
        staking.getReward();
        vm.warp(block.timestamp + 1);
        uint256 before = BPLN.balanceOf(address(owner));
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(BPLN), 200);
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(BPLN), 200);
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(BPLN), 200);
        ...
        gauge.batchRewardPerToken(address(BPLN), 200);
        vm.warp(block.timestamp + 1);
        uint256 earned = gauge.earned(address(BPLN), address(owner));
        address[] memory rewards = new address[](1);
        rewards[0] = address(BPLN);
        vm.warp(block.timestamp + 1);
        gauge.getReward(address(owner), rewards);
        vm.warp(block.timestamp + 1);
        uint256 after_ = BPLN.balanceOf(address(owner));
    ...
    function minterMint2() public {
        ...
        token[0] = address(BPLN);
        tokens[0] = token;
        voter.claimRewards(gauges, tokens);
        assertEq(gauge.rewardRate(address(BPLN)), staking.rewardRate());
        console2.log(gauge.rewardPerTokenStored(address(BPLN)));
    }
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pair), address(gauge), PAIR_1);
        owner3.deposit(address(gauge), PAIR_1, 0);
        uint256 before = BPLN.balanceOf(address(owner3));
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(BPLN), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(BPLN);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
        uint256 after_ = BPLN.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
        console2.log(gauge.rewardPerTokenStored(address(BPLN)));
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        BPLN.approve(address(gauge), BPLN.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(BPLN), BPLN.balanceOf(address(owner)));
