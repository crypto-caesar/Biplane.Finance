...
import "contracts/Bpln.sol";
import "contracts/BiplaneLibrary.sol";
...
abstract contract BaseTest is Test, TestOwner {
    ...
    function mintBpln(address[] memory _accounts, uint256[] memory _amounts) public {
        for (uint256 i = 0; i < _amounts.length; i++) {
            BPLN.mint(_accounts[i], _amounts[i]);
        }
    }
    ...
    function deployPairFactoryAndRouter() public {
        ...
        lib = new BiplaneLibrary(address(router));
    }
