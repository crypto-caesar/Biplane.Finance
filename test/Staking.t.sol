...
contract StakingTest is BaseTest {
    ...
    function deployBaseCoins() public {
        ...
        escrow = new TestVotingEscrow(address(BPLN));
        voter = new TestVoter();
    }
    ...
    function createLock2() public {
        ...
        owner2.approve(address(BPLN), address(escrow), TOKEN_1);
    ...
    function createLock3() public {
        ...
        owner3.approve(address(BPLN), address(escrow), TOKEN_1);
    ...
    function deployFactory() public {
        ...
        staking = new TestStakingRewards(address(stake), address(BPLN));
    }
    ...
    function depositEmpty() public {
        ...
        assertEq(gauge.earned(address(BPLN), address(owner)), staking.earned(address(owner)));
    }
    ...
    function depositEmpty2() public {
        ...
        assertEq(gauge.earned(address(BPLN), address(owner2)), staking.earned(address(owner2)));
    }
    ...
    function depositEmpty3() public {
        ...
        assertEq(gauge.earned(address(BPLN), address(owner3)), staking.earned(address(owner3)));
    }

    function notifyRewardsAndCompare() public {
        depositEmpty3();

        BPLN.approve(address(staking), TOKEN_1M);
        BPLN.approve(address(gauge), TOKEN_1M);
        assertEq(staking.rewardPerTokenStored(), gauge.rewardPerTokenStored(address(BPLN)));
        staking.notifyRewardAmount(TOKEN_1M);
        gauge.notifyRewardAmount(address(BPLN), TOKEN_1M);
    ...
    function notifyRewardsAndCompareOwner1Again() public {
        ...
        gauge.batchRewardPerToken(address(BPLN), 200);
        assertEq(staking.rewardPerTokenStored(), gauge.rewardPerTokenStored(address(BPLN)));
        // uint256 sb = BPLN.balanceOf(address(owner));
        staking.getReward();
        // uint256 sa = BPLN.balanceOf(address(owner));
        // uint256 gb = BPLN.balanceOf(address(owner));
        address[] memory tokens = new address[](1);
        tokens[0] = address(BPLN);
        gauge.getReward(address(owner), tokens);
        // uint256 ga = BPLN.balanceOf(address(owner));
        vm.warp(block.timestamp + 604800);
        vm.roll(block.number + 1);
        staking.withdraw(1e21);
        gauge.withdraw(1e21);
