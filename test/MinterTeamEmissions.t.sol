...
contract MinterTeamEmissions is BaseTest {
    ...
    function setUp() public {
        ...
        deployCoins();
        mintStables();
        uint256[] memory amountsBpln = new uint256[](2);
        amountsBpln[0] = 1e25;
        amountsBpln[1] = 1e25;
        mintBpln(owners, amountsBpln);
        team = new TestOwner();
        ...
        BPLN.approve(address(router), TOKEN_1);
        ...
        address pair = router.pairFor(address(FRAX), address(BPLN), false);

        BPLN.approve(address(voter), 5 * TOKEN_100K);
        ...
        assertEq(BPLN.balanceOf(address(minter)), 14 * TOKEN_1M);
        ...
        vm.roll(block.number + 1);
        before = BPLN.balanceOf(address(owner));
        minter.update_period(); // initial period week 2
        after_ = BPLN.balanceOf(address(owner));
    ...
    function testTeamEmissionsRate() public {
        ...
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        uint256 beforeTeamSupply = BPLN.balanceOf(address(team));
        uint256 weekly = minter.weekly_emission();
        uint256 growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        uint256 afterTeamSupply = BPLN.balanceOf(address(team));
        uint256 newTeamBpln = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamBpln) * 30) / 1000, newTeamBpln); // check 3% of new emissions to team

        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        beforeTeamSupply = BPLN.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        afterTeamSupply = BPLN.balanceOf(address(team));
        newTeamBpln = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamBpln) * 30) / 1000, newTeamBpln); // check 3% of new emissions to team

        // rate is right even when BPLN is sent to Minter contract
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        owner2.transfer(address(BPLN), address(minter), 1e25);
        beforeTeamSupply = BPLN.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
    ...
    function testChangeTeamEmissionsRate() public {
        ...
        uint256 afterTeamSupply = BPLN.balanceOf(address(team));
        uint256 newTeamBpln = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamBpln) * 50) / 1000, newTeamBpln); // check 5% of new emissions to team
    }
}
